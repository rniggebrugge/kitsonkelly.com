	...
	return declare("dojox.gantt.GanttChart", [], {
		...
		loadJSONData: function(filename){
			...
			request.get(_this.dataFilePath, {
				...
				0 && console.log("Successfully! Loaded data from: " + _this.dataFilePath);
			}, function(err){	
				0 && console.log("Failed! Load error: " + _this.dataFilePath);
			});
		},
			...
			try {
				request.post(_this.saveProgramPath, {
					...
					if((util.checkStatus(response.options.status))||
						...
						0 && console.log("Successfully! Saved data to " + _this.dataFilePath);
					}else{
						0 && console.log("Failed! Saved error");
					}
				});
			} catch(e){
				0 && console.log("exception: " + e.message);
			}
		},
